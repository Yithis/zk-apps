...
impl ContractInteractionCommand {
    pub fn get_metadata_file(&self) -> PathBuf {
        match self {
            ...
            ContractInteractionCommand::Vote(VoteCmd { metadata_file, .. }) => {
                metadata_file.clone()
            }
        }
    }
...
pub struct VoteCmd {
    ...
    /// File with raw proving key bytes for vote.
    ///
    /// If not found, command will fail - the tool won't generate it for you.
    #[clap(default_value = "vote.pk.bytes", value_parser = parsing::parse_path)]
    pub vote_key_file: PathBuf,
}
