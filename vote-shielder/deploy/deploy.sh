...
export CLIAIN_IMAGE="cliain-liminal:latest"
...
generate_relation_keys() {
  $DOCKER_SH \
    ...
    -w "/workdir" \
    "${CLIAIN_IMAGE}" \
    -c "RUST_BACKTRACE=1 /usr/local/bin/cliain snark-relation generate-keys ${1} ${2:-}"
...
docker_ink_dev() {
  docker run --rm \
    ...
    -v ~/aleph/aleph-node:/usr/local/aleph-node \
    -v ~/aleph/crypto:/usr/local/crypto \
    -v ~/aleph/poseidon377:/usr/local/poseidon377 \
    --network host \
    --entrypoint /bin/sh \
    "${INK_DEV_IMAGE}" \
    -c "${1}"
